...
def main():
    ...
    for name, param in model.named_parameters():
        if "backbone" in name:
            debug_print(logger, name + " --requires grad: " + str(param.requires_grad))

    # modules that should be always set in eval mode
    # their eval() method should be called after model.train() is called
    eval_modules = (model.rpn, model.backbone, model.roi_heads.box, model.rel_backbone.body.layer2, model.rel_backbone.body.layer3)
    ...
    for name, param in model.named_parameters():
        if "backbone" in name:
            debug_print(logger, name + " requires grad: " + str(param.requires_grad))
        if "backbone.body.layer4.0.conv1.weight" in name:
            debug_print(logger, "=" * 20)
            debug_print(logger, name + str(type(param)) + str(type(param.data)))
            debug_print(logger, name + str(param.data[0]))
            debug_print(logger, "=" * 20)
